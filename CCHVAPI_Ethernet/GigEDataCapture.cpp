...
int GigECDataCapture::Open(int height, int width)// 
{
	...
	int sztemp=sizeof(tempsin);
	int rst = sendto(socketSrv, sendbuff, 32, 0, (struct sockaddr*)&addrClient, sizeof(struct sockaddr_in));//stop
	usleep(10);
	sendbuff[3] = 0x01;//start
	rst = sendto(socketSrv, sendbuff, 32, 0, (struct sockaddr*)&addrClient, sizeof(struct sockaddr_in));
	if (rst < 0)
	{
		return errno;
	}
...
int GigECDataCapture::initUDP(int *s)
{
	...
	*s = -1;
	...
	addrSrv.sin_addr.s_addr = this_clientProp->getpcIP();
	unsigned long long temp = htonl(inet_addr("192.168.2.7"));
	addrSrv.sin_family = AF_INET;
	addrSrv.sin_port = htons(8080);

	addrClient.sin_addr.s_addr = htonl(this_clientProp->getcamIP());
...
void*  GigECDataCapture::getUdpDataThreadAdapter(void *handle)
{
	GigECDataCapture* pThis = (GigECDataCapture*)handle;
	pThis->getUdpDataThreadFunc();
	return 0;
}
void GigECDataCapture::getUdpDataThreadFunc()
{
	long nRet;
	unsigned char* recv_buf = new unsigned char[PACKSIZE];
	socklen_t cliaddr_len = sizeof(client_addr1);
	long already_get_num = 0;
	char resendbuf[32];
	memset(resendbuf,0,32);
...
void GigECDataCapture::pack2FrameThreadFunc()
{
	long nRet = 0;
	char recv_buf[1400] = "";
...
void *GigECDataCapture::pack2FrameThreadAdapter(void* __this)
{
	GigECDataCapture* pThis = (GigECDataCapture*)__this;
	pThis->pack2FrameThreadFunc();
	return 0;
}
...
cq_uint32_t GigECDataCapture::WriteReg(cq_uint32_t addr, cq_uint32_t data)
{
	char *sendbuff = new char[32];
