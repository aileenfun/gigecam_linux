...
GigECDataProcess::GigECDataProcess(GigEwqueue<GigEimgFrame*>&queue,void *lpUser)
	:cpm_queue(queue)
{
	m_hThread=NULL;
	lpcb=lpUser;
...
void* GigECDataProcess::ThreadAdapter(void* __this)
{
	GigECDataProcess* _this=(GigECDataProcess*) __this;
	_this->ThreadFunc();	
...
void GigECDataProcess::ThreadFunc()
{
	int iRet=0;
	int endflag = 1;
	m_bEnd=false;
...
int GigECDataProcess::OutPutWrapper(LPMV_CALLBACK2 CallBackFunc,void* lpUser)
{
...
{
	tagRGB* TempRgbBuf = new tagRGB[g_width];//����õ�������ָ���ַѭ��ʹ��
