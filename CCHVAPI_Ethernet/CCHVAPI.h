...
public :
	int b_connected;
public:
	GigEcamInstance(void*lp,LPMV_CALLBACK2 CallBackFunc):m_DeviceGVCP()
	{
	...
	GigEcamInstance(void*lp,LPMV_CALLBACK2 CallBackFunc,CCHCamera *info):m_DeviceGVCP()
	{
	...
	int ReadReg(unsigned int addr,uint32_t *data)
	{
		int rst= m_DeviceGVCP.ReadReg(addr, data);
		return rst;
	}
	unsigned int WriteReg(unsigned int addr,unsigned int data)
	{
...
// int initCCTAPI(int camNum);
 ...
 int GigEgetDataCnt(int camNum=1);
 long GigEgetErrPackCnt(int camNum=1);
 cq_int32_t GigEsetTrigMode(cq_int32_t s,cq_int32_t camNum=1);
 int GigEsendSoftTrig(int camNum=1);
 int GigEsetIP(CCHCamera *devinfo,int camNum=1);
 int GigEforceIP(CCHCamera *devinfo);
 int GigEcloseConnection(int &camNum);
 int GigEsearchCamera(map_camera * info);
 unsigned int GigEWriteReg(unsigned int addr, unsigned int data,int camNum=1);
 unsigned int GigEReadReg(unsigned int addr,uint32_t *data,int camNum=1);
 int GigEsetGain(uint32_t value,int isauto,int camNum=1);
 int GigEsetExpo(uint32_t value,int isauto,int camNum=1);
 int GigEsetFreq(uint32_t value,int camNum=1);
 int GigEsetWB(uint32_t rvalue,uint32_t gvalue,uint32_t g2value,uint32_t bvalue,int camNum=1);
 int GigEsetCamSize(int camsize,int camNum=1);
